...
class HashGenerator {
    ...
    static public function generate($length = 12) {
    ...
    public static function udihash($num, $length, $len = 7) {
        $ceil = pow(62, $len);
        $prime = self::$golden_primes[$len];
        $dec = ($num * $prime)-floor($num * $prime/$ceil)*$ceil;
        $hash = self::base62($dec);
        $res = str_pad($hash, $len, "0", STR_PAD_LEFT);
        $str = str_replace("0", "4", str_replace("o", "p", str_replace("O", "P", str_replace("l", "q", str_replace("I", "Q", $res)))));
        $str .= self::generate($length - strlen($str));
        return $str;
    }
}
